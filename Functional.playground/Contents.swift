...
//:
//: IMPORTANT - make sure you view the assistant editor to see the graphs
...
//:
//: The nested functions captures `offset`, an additional Point2D (2D vector) used as an offset
//: As the `offset` parameter is not mutated outside the nested function, then a copy is captured

func translate(offset: Point2D) -> Transform2D {
    
    //: This function is returned - a 2D Point is passed in, the captured offset will be added and a new point returned.
...
let plot2 : PlotView = PlotView(frame: f, points: points2)
//XCPShowView("Plot 2 - Composite Transform", view: plot2)
...
let plot3 : PlotView = PlotView(frame: f, points: points3)
//XCPShowView("Plot 3 - Composite Transform", view: plot3)
...
func orbit_nicecurry(center : Point2D)(angle: Double) -> Transform2D {
    let minusCenter = negate(center)
    let tx = translate(minusCenter) |-> rotate(angle) |-> translate(center)
    return tx
}
...
func orbit_notcurried(center : Point2D, angle: Double) -> Transform2D {
    let minusCenter = negate(center)
    let tx = translate(minusCenter) |-> rotate(angle) |-> translate(center)
    return tx
}
...
let newPosition = orbit(sun)(90.0)(planet) //Curried version
let points4 = ["Sun" : sun, "Planet" : planet, "Planet+90" : newPosition]
let plot4 = PlotView(frame: f, points: points3)
XCPShowView("Plot 4 - Orbit Transform", view: plot4)
...
let plot5 = PlotView(frame: f, points: planetOrbits)
//XCPShowView("Plot 5 - Two Planets", view: plot5)
